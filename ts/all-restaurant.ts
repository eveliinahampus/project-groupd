...
let restaurant_data_allRestaurants = [];
fetch("http://localhost:3001/api/restaurants")
  .then((response) => response.json())
  .then((json) => {
    console.log("full json", json);
    restaurant_data_allRestaurants = json;

    console.log("restaurant_data_allRestaurants", restaurant_data_allRestaurants.length);

    //show all restaurants
    if (restaurant_data_allRestaurants) {
      // excute only if restaurant_data is not undefined
      for (let i = 0; i < restaurant_data_allRestaurants.length; i++) {
        console.log("-------name", restaurant_data_allRestaurants[i].restaurant_name);
      }

      const sortedRestaurant = restaurant_data_allRestaurants.sort((a, b) => {
        return b.average_stars - a.average_stars;
      });
      for (let i = 0; restaurant_data_allRestaurants.length ; i++) {
        renderRestaurantCard2(sortedRestaurant[i]);
...
//DOM
const parentID6 = "allRestaurant";
function renderRestaurantCard2(restaurant_data_allRestaurants: any) {
  if (!restaurant_data_allRestaurants) return; // checking null
  ...
  h5Element.textContent = restaurant_data_allRestaurants.average_stars;
  ...
  h6Element.textContent = restaurant_data_allRestaurants.reviews.length  ;
  ...
  h5Element2.textContent = `${restaurant_data_allRestaurants.restaurant_name}`
  //change color to
  h5Element2.style.color = "#EB6440";

  // create p element with text content "Pohjoisesplanadi 5 ,Helsinki , 00100"
  const pElement = document.createElement("p");
  pElement.textContent = `${restaurant_data_allRestaurants.street_address}  ,${restaurant_data_allRestaurants.city}  , ${restaurant_data_allRestaurants.zip_code} `

  // create p element with text content "+358 10 320 6250"
  const pElement2 = document.createElement("p");
  pElement2.textContent = `${restaurant_data_allRestaurants.phone_number}`
  ...
  const parentElement = document.getElementById(parentID6);
  if (parentElement) {
